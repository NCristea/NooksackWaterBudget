...
int inputT(int initT[], int iend[], int &Neq, vector<vector<double> > &Qact, double acTime[],
           string &modelId, int &Npar, const int Nrx, const int iex)
{
    ...
    vector<vector<double> > wrg1(maxSlp,vector<double>(maxGauge));
...
L150:
    ...
    for (i = 0; i < max(Neq, 1); i++) {
        initT[i] = 1;
        iend[i] = model2::m;
        if ( iend[i] <= Nrx ) {
            for (j = initT[i]; j <= iend[i]; j++) {
                Qact[j-1][i] = flow[i][j-1];
                //  p[j-1] = PREC(1,j)
            }
        }
        else {
...
L150:
        ...
        // ALLOCATE DYNAMIC ARRAYS
        vector<int> lake_reach(max_lakes);
    ...
    if (iflag == 1) {    //  Only do work first time through
        ...
        ndump = 0;
        setup_zbar0s(model1::Ns, Si, Sp, tl, model2::interval, lOut, iReach, dFlow, knt, maxResponse, maxSlp);
    ...
    for (i1 = 1; i1 <= model5::Neq1; i1++) {
        ...
        for (i4 = 1; i4 <= model1::Ns; i4++) {
            if (iReach[i4-1] == lOut[i1-1]) {
                suma[i1-1] += Spd[0][i4-1]; // area in mm*2
                knt[i1-1] = knt[i1-1] + 1;
            }
        }
    }
